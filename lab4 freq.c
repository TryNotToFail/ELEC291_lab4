...
void main (void) 
{
	...
	printf ("EFM8 Frequency and Period measurement using Timer/Counter 0.\n"
	        "File: %s\n"
	        "Compiled: %s, %s\n\n",
	...
	while(1)
	{
		...
		cap=1.44*1000000.0/(4.5*F);
		capf = cap-1.0;
		period=1000000.0/F;
		Ff=F/1000.0;
		
		sprintf(capa,"C=%lunF P=%.1fus", capf, period);
		sprintf(freq,"F=%.3fkHz", Ff);
		LCDprint(freq, 1, 1);
		LCDprint(capa, 2, 1);
	}
